...
class _CommissionsPageState extends State<CommissionsPage> {
  ...
  Future<void> _loadPreviewPreference() async {
    final visible = await CommissionUiPreferences.getCommissionsPreviewVisible();
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _toggleCollectionsPreview() async {
    ...
    await CommissionUiPreferences.setCommissionsPreviewVisible(next);
  }
        ...
        children: [
          ...
          Card(
            child: ListTile(
              ...
              subtitle: const Text(
                'Registra i valori del mese corrente per agente/collaboratore.',
              ),
              trailing: const Icon(Icons.chevron_right),
              onTap: () async {

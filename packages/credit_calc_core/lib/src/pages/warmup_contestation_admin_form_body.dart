...
/// Form admin — crea o modifica contestazioni warm-up con schede AI.
class WarmupContestationAdminFormBody extends StatefulWidget {
  const WarmupContestationAdminFormBody({
    super.key,
    this.existing,
    this.initialContext = WarmupContestationContextCore.sollecito,
  });

  final WarmupContestationCore? existing;
  final WarmupContestationContextCore initialContext;

  @override
  State<WarmupContestationAdminFormBody> createState() =>
      _WarmupContestationAdminFormBodyState();
}

class _WarmupContestationAdminFormBodyState
    extends State<WarmupContestationAdminFormBody> {
  final _formKey = GlobalKey<FormState>();
  late final TextEditingController _declaredCtrl;
  late WarmupContestationContextCore _context;
  late WarmupContestationStatusCore _status;
  ...
  void initState() {
    ...
    _status = existing?.status ?? WarmupContestationStatusCore.approved;
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      await WarmupContestationAdminCore.save(
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
          ...
          DropdownButtonFormField<WarmupContestationContextCore>(
            value: _context,
            decoration: const InputDecoration(
              labelText: 'Contesto',
              border: OutlineInputBorder(),
            ),
            items: WarmupContestationContextCore.values
                .map(
          ...
          DropdownButtonFormField<WarmupContestationStatusCore>(
            value: _status,
            decoration: const InputDecoration(
              labelText: 'Stato',
              border: OutlineInputBorder(),
            ...
            items: const [
              WarmupContestationStatusCore.approved,
              WarmupContestationStatusCore.pendingReview,
              WarmupContestationStatusCore.draft,
            ]
                .map(

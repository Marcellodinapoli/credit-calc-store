...
class _CommissionSettingsPageState extends State<CommissionSettingsPage> {
  ...
  Future<void> _save() async {
    ...
    try {
      final creditorSnap = await CommissionCreditorDataAccess.instance
          .loadCreditor(_creditorId);
      final existingSettings = creditorSnap?['commissionSettings'];
      final hasSchema = existingSettings is Map && existingSettings.isNotEmpty;
      if (!hasSchema) {
        if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return wrapCreditCalcPage(
      ...
      body: SectionLockScope(
        sectionKey: 'commission_settings:${widget.creditorId}',
        child: ColoredBox(
